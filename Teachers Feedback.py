    ...
    if user_info["role"] == "Admin":
        ...
        if teachers:
            with st.form("feedback_form", clear_on_submit=True):
                tid = st.selectbox("Select Teacher", list(teachers.keys()), format_func=lambda x: f"{teachers[x]} ({x})")
                mc1, mc2 = st.columns(2)
                with mc1: ml = st.selectbox("Lecturing", [5,4,3,2,1]); md = st.selectbox("Discipline", [5,4,3,2,1]); mp = st.selectbox("Portion", [5,4,3,2,1])
                with mc2: mi = st.selectbox("Impression", [5,4,3,2,1]); mc = st.selectbox("Communication", [5,4,3,2,1])
                rev = st.text_area("Comments")
                if st.form_submit_button("Submit"):
                    avg = (ml + md + mp + mi + mc) / 5
                    perf = "Good" if avg >= 4.0 else ("Moderate" if avg >= 2.5 else "Low")
                    conn = get_active_conn(); cur = conn.cursor()
                    cur.execute("INSERT INTO feedback (teacher_id, teacher_name, student_name, performance, stars, review) VALUES (%s, %s, %s, %s, %s, %s)",
                                (tid, teachers[tid], user_info["name"], perf, round(avg), f"[L:{ml}, D:{md}, P:{mp}, I:{mi}, C:{mc}] {rev}"))
                    conn.commit(); cur.close(); st.success("Submitted!"); st.rerun()
        ...
        if not df_teacher.empty:
            st.metric("Average Rating", f"{round(df_teacher['stars'].mean(), 2)} / 5.0")
